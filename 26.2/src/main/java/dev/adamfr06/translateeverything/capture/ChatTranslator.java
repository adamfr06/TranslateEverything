...
public final class ChatTranslator {
    private record Pending(Component original, MessageSignature signature, GuiMessageTag indicator,
    ...
    public static boolean onChatMessage(Component message, MessageSignature signature, GuiMessageTag indicator) {
        try {
            ...
            if (cfg.hideSameLanguage
                ...
                addLine(Minecraft.getInstance(), message.copy().append(forceSuffix(id)), signature, indicator);
                return true;
            }
    ...
    public static void flushAll(Minecraft client) {
        Pending pending;
    ...
    private static void release(Minecraft client, Pending pending, TranslationService.Result result) {
        TEConfig cfg = TEConfig.get();
        ...
        if (mode == TEConfig.ChatMode.REPLACE) {
            ...
            MutableComponent line = Component.literal(result.translatedText()).setStyle((colored == null ? Style.EMPTY : colored)
            ...
            line.append(Component.literal(" ⧉").setStyle(Style.EMPTY
                    .withColor(0x555555)
                    .withClickEvent(new ClickEvent.CopyToClipboard(result.translatedText()))
                    .withHoverEvent(new HoverEvent.ShowText(Component.literal("Copy the translation")))));
            line.append(askAiSuffix(dev.adamfr06.translateeverything.translate.ChatContext.registerTranslated(
                    pending.plainText(), result.translatedText(), false)));
    ...
    private static MutableComponent translationLine(TranslationService.Result result, String prefix, String original) {
        int id = dev.adamfr06.translateeverything.translate.ChatContext.registerTranslated(
                original, result.translatedText(), false);
        return Component.literal(prefix).withStyle(s -> s.withColor(0x557799))
                .append(Component.literal(result.translatedText()).setStyle(Style.EMPTY
                        .withColor(0xB8C8D8)
                        .withClickEvent(new ClickEvent.CopyToClipboard(result.translatedText()))
                        .withHoverEvent(new HoverEvent.ShowText(
                                Component.literal(languageTag(result) + " · click to copy the translation")
                                        .withStyle(s -> s.withColor(0x777777))))))
                .append(askAiSuffix(id));
    }

    /** A small clickable icon that opens the AI follow-up box for this exact message. */
    private static MutableComponent askAiSuffix(int id) {
        return Component.literal(" ⟳").setStyle(Style.EMPTY
                .withColor(0x5AC8E0)
                .withClickEvent(new ClickEvent.RunCommand("/teai " + id))
                .withHoverEvent(new HoverEvent.ShowText(
                        Component.literal("Ask the AI about this message").withStyle(s -> s.withColor(0x9AD8E8)))));
    }

    /** Clickable icon on a line that was skipped as already readable; translates it on demand. */
    private static MutableComponent forceSuffix(int id) {
        return Component.literal(" ⇄").setStyle(Style.EMPTY
                .withColor(0xE6B15E)
                .withClickEvent(new ClickEvent.RunCommand("/teforce " + id))
                .withHoverEvent(new HoverEvent.ShowText(
                        Component.literal("Translate this line").withStyle(s -> s.withColor(0xE6B15E)))));
    }

    /** Translates a previously skipped chat line and prints the result beneath it. */
    public static void forceTranslate(Minecraft client, int id) {
    ...
    private static Component withCopy(Component original, String toCopy, String hint) {
        return Component.empty().setStyle(Style.EMPTY
                        .withClickEvent(new ClickEvent.CopyToClipboard(toCopy))
                        .withHoverEvent(new HoverEvent.ShowText(
                                Component.literal(hint).withStyle(s -> s.withColor(0x777777)))))
                .append(original);
    }

    private static void addLine(Minecraft client, Component text,
        ...
        try {
            ...
            TranslateEverythingClient.LOGGER.error("[Chat] addClientSystemMessage failed", t);
        } finally {
            internalAdd = false;
        }
    }
    ...
    private static void markEmitted(Component text) {
        ...
        if (key != null && !key.isBlank()) {
            ...
            if (original != null && !original.isBlank()) {
                ...
                if (selfOriginal.size() > 64) {
                    selfOriginal.clear(); // tiny, short-lived map; a hard reset is fine
                }
            }
    ...
    private static void showConflictWarning(Minecraft client) {
        Component warning = Component.empty()
                .append(Component.literal("⚠ TranslateEverything: ").withStyle(s -> s.withColor(0xFFAA00)))
                .append(Component.literal(conflictMod + " also translates chat. Running both duplicates work and can loop "
                        + "messages. Disable " + conflictMod + " or turn off chat translation here.").withStyle(s -> s.withColor(0xFFDD88)));
