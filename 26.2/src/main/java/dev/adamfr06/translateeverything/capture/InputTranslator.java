...
public final class InputTranslator {
    ...
    public static void translateFocusedField(Minecraft client, EditBox field) {
        String selected = field.getHighlighted();
        boolean whole = selected == null || selected.isEmpty();
        String text = whole ? field.getValue() : selected;
        if (text == null || text.isBlank()) {
            CaptureManager.feedback(client, "Nothing to translate in this field");
            return;
        }
        String fieldSnapshot = field.getValue();
        int cursorSnapshot = field.getCursorPosition();
        Object screenSnapshot = client.gui.screen();
        ...
        TranslationService.translate(text, TranslationService.homeLanguage(), target, TEConfig.get().inputEngine, null)
                .thenAccept(result -> client.execute(() -> {
                    if (client.gui.screen() != screenSnapshot || !field.getValue().equals(fieldSnapshot) || field.getCursorPosition() != cursorSnapshot || !java.util.Objects.equals(field.getHighlighted(), selected)) return;
                    if (result.error()) {
                    ...
                    if (whole) {
                        ...
                        field.insertText(out); // replaces the current selection
                    }
                }));
    }
    ...
    public static void updatePreview(String currentInput) {
        ...
        Typed typed = typed(input);
        if (!typed.translatable() || typed.core().isBlank()) {
            computing = false;
            return;
        }
            ...
            if (input.equals(pinnedOriginal) && pinnedTranslation != null && !pinnedTranslation.isBlank()) {
                ...
                if (TEConfig.get().inputRoundTrip) {
                    TranslationService.backTranslate(forwardText, resolveTarget(), TranslationService.homeLanguage(),
                            ...
                            .thenAccept(rt -> Minecraft.getInstance().execute(() -> {
        ...
        if (sess != null) {
            ...
            sess.askHidden("I edited my message to: \"" + core + "\". Re-translate it into " + sess.tgtName
                    ...
                    .thenAccept(reply -> Minecraft.getInstance().execute(() -> {
    ...
    private static void coldPreview(String input, String core, String target, java.util.List<String> ctx) {
        ...
        TranslationService.translate(core, TranslationService.homeLanguage(), target, TEConfig.get().inputEngine, ctx, PREVIEW_TAG)
                .thenAccept(fwd -> Minecraft.getInstance().execute(() -> {
                    if (request != previewRevision || !input.equals(shownInput)) {
                        return; // superseded by newer input
                    }
                    computing = false;
    ...
    private static void afterForward(String input, String core, String target, java.util.List<String> ctx) {
        ...
        if (TEConfig.get().inputFaithfulnessCheck) {
            ...
            TranslationService.checkFaithfulness(core, forwardText, target, ctx)
                    .thenAccept(f -> Minecraft.getInstance().execute(() -> {
                        if (request == previewRevision && input.equals(shownInput)) {
    ...
    public static void renderPreviewBar(GuiGraphicsExtractor context, String currentInput, int screenWidth, int screenHeight) {
        var client = Minecraft.getInstance();
        var wrapped = previewLines(currentInput, screenWidth);
        if (wrapped.isEmpty()) return;
        int lineH = 11;
        int barBottom = screenHeight - 22;
        int visible = Math.min(wrapped.size(), Math.max(2, (screenHeight / 2 - 22) / lineH));
        int y = barBottom - visible * lineH - 2;
        context.fill(2, y - 2, screenWidth - 2, barBottom - 2, 0xD0000000);
        for (int i = 0; i < visible; i++) {
            context.text(client.font, wrapped.get(i), 6, y + i * lineH, 0xFFFFFFFF, true);
        }
    }

    private static java.util.List<net.minecraft.util.FormattedCharSequence> previewLines(String currentInput, int screenWidth) {
        TEConfig cfg = TEConfig.get();
        if (!active() || cfg.inputMode != TEConfig.InputMode.PREVIEW) {
            return java.util.List.of();
        }
        updatePreview(currentInput);
        Typed typed = typed(currentInput);
        if (!typed.translatable() || typed.core().isBlank()) {
            return java.util.List.of();
        }
        String addressed = typed.recipient().isBlank() ? "" : "to " + typed.recipient() + ": ";
        Minecraft client = Minecraft.getInstance();
        String target = resolveTarget().toUpperCase(Locale.ROOT);
        java.util.List<Component> lines = new java.util.ArrayList<>();
        if (forwardText.isBlank()) {
            lines.add(Component.literal("-> " + target + "  ").withStyle(s -> s.withColor(0x66CCFF))
                    .append(Component.literal(computing ? "translating…" : "…").withStyle(s -> s.withColor(0x888888))));
        } else {
            lines.add(Component.literal("-> " + target + "  ").withStyle(s -> s.withColor(0x66CCFF))
                    .append(Component.literal(addressed).withStyle(s -> s.withColor(0xE6B15E)))
                    .append(Component.literal(forwardText).withStyle(s -> s.withColor(0xFFFFFF))));
            if (cfg.inputRoundTrip && !roundTripText.isBlank()) {
                ...
                lines.add(Component.literal("-> " + back + "  ").withStyle(s -> s.withColor(0x77AA77))
                        .append(Component.literal(roundTripText).withStyle(s -> s.withColor(0xBBBBBB))));
            }
            if (stickyApplied) {
                lines.add(Component.literal("Reviewed").withStyle(s -> s.withColor(0x4FC1A6)));
            }
        ...
        for (Component line : lines) wrapped.addAll(client.font.split(line, Math.max(80, screenWidth - 20)));
        return wrapped;
    }

    public static int chatPreviewSpace() {
        var client = Minecraft.getInstance();
        if (!(client.gui.screen() instanceof ChatDraftAccess access)) return 0;
        int height = client.getWindow().getGuiScaledHeight();
        int lines = previewLines(access.translateeverything$draft(), client.getWindow().getGuiScaledWidth()).size();
    ...
    private static java.util.List<Component> faithLines(Minecraft client, int maxW) {
        int col;
        String text;
        switch (faithVerdict) {
        ...
        for (String word : text.split(" ")) {
            ...
            if (client.font.width(probe) > maxW && !line.isEmpty()) {
        ...
        java.util.List<Component> out = new java.util.ArrayList<>(wrapped.size());
        for (String l : wrapped) {
            out.add(Component.literal(l).withStyle(s -> s.withColor(fc)));
        }
        return out;
    }
    ...
    public static boolean interceptSend(Minecraft client, String content) {
        TEConfig cfg = TEConfig.get();
        ...
        if (cfg.inputMode == TEConfig.InputMode.PREVIEW && content.equals(shownInput)
            ...
            resetPreview();
            doSend(client, envelope + approved, core, approvedBack);
            return true;
        }
        ...
        TranslationService.translate(core, TranslationService.homeLanguage(), target, cfg.inputEngine, ctx)
                ...
                .thenAccept(result -> client.execute(() -> {
                    if (connection != client.getConnection()) return;
                    ...
                    String toSend = envelope + (failed ? core : result.translatedText());
                    doSend(client, toSend, core);
                }));
        return true;
    }

    private static void doSend(Minecraft client, String message) {
        doSend(client, message, null, null);
    }

    private static void doSend(Minecraft client, String message, String original) {
    ...
    private static void doSend(Minecraft client, String message, String original, String readBack) {
        ...
        refineSession = null; // the thought completed; its review context retires with it
    ...
    public static void sendTranslated(Minecraft client, String text) {
        if (text != null && !text.isBlank()) {
            doSend(client, text.strip());
        }
    }

    public static void sendReviewed(Minecraft client, String text, String original, String back) {
    ...
    public static void openConfirm(Minecraft client, net.minecraft.client.gui.screens.Screen parent, String draft) {
        ...
        if (resumed != null) {
            ...
            client.setScreenAndShow(new dev.adamfr06.translateeverything.gui.AiChatScreen(parent, resumed));
            return;
        }
        ...
        client.setScreenAndShow(new dev.adamfr06.translateeverything.gui.AiChatScreen(parent, convo));
    }

    /** Opens the follow-up box for the most recently translated incoming message. */
    public static void openFollowup(Minecraft client) {
        ...
        dev.adamfr06.translateeverything.translate.AiConversation convo =
                dev.adamfr06.translateeverything.translate.AiConversation.followup(
                        last.original(), last.translation(), "the original language", tgtName, tgtName,
                        dev.adamfr06.translateeverything.translate.ChatContext.window(last.original()));
        client.setScreenAndShow(new dev.adamfr06.translateeverything.gui.AiChatScreen(null, convo));
    }

    /** Opens the follow-up box for a specific chat message (clicked in chat). */
    public static void openFollowupFor(Minecraft client, int id) {
        ...
        dev.adamfr06.translateeverything.translate.AiConversation convo =
                dev.adamfr06.translateeverything.translate.AiConversation.followup(
                        t.original(), t.translation(), "the original language", tgtName, tgtName,
                        dev.adamfr06.translateeverything.translate.ChatContext.window(t.original()));
        client.setScreenAndShow(new dev.adamfr06.translateeverything.gui.AiChatScreen(null, convo));
    }
}
