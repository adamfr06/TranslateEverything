...
public final class GameLanguages implements MinecraftGlossary.LangSource {
    ...
    private static String resolve(String isoCode) {
        ...
        for (String code : client.getLanguageManager().getLanguages().keySet()) {
    ...
    private static Map<String, String> read(String file) {
        ...
        Identifier id = Identifier.fromNamespaceAndPath("minecraft", "lang/" + file + ".json");
        List<Resource> stack = client.getResourceManager().getResourceStack(id);
        if (stack.isEmpty()) {
            return null;
        }
