...
public class ConfigScreen extends Screen {
    ...
    private TabNavigationWidget tabBar;
    private int selectedTab = 0;

    public ConfigScreen(Screen parent) {
        super(Text.literal("TranslateEverything"));
        this.parent = parent;
    }
    ...
    private ButtonWidget toggle(String label, Supplier<Boolean> get, Consumer<Boolean> set) {
        return ButtonWidget.builder(onOff(label, get.get()), b -> {
            boolean now = !get.get();
            set.accept(now);
            b.setMessage(onOff(label, now));
        }).dimensions(0, 0, SettingsList.CONTROL_WIDTH, 20).build();
    }

    private static Text onOff(String label, boolean on) {
        return Text.literal(label + ": ").append(Text.literal(on ? "ON" : "OFF")
                .styled(s -> s.withColor(on ? 0x7FE08A : 0xE0827F)));
    }

    /** A button that steps through a fixed set of choices, vanilla-options style. */
    private <T> ButtonWidget cycle(String label, T[] values, Supplier<T> get, Consumer<T> set,
                             java.util.function.Function<T, String> name) {
        ButtonWidget[] self = new ButtonWidget[1];
        self[0] = ButtonWidget.builder(Text.literal(label + ": " + name.apply(get.get())), b -> {
            int i = 0;
            ...
            set.accept(next);
            b.setMessage(Text.literal(label + ": " + name.apply(next)));
        }).dimensions(0, 0, SettingsList.CONTROL_WIDTH, 20).build();
        return self[0];
    }

    private ButtonWidget engines(String label, Supplier<TEConfig.Engine> get, Consumer<TEConfig.Engine> set) {
        return cycle(label, TEConfig.Engine.values(), get, set, e -> e.label);
    }

    private SliderWidget slider(String label, double min, double max, double step, String unit,
                                        DoubleSupplier get, Consumer<Double> set) {
        return new SliderWidget(0, 0, SettingsList.CONTROL_WIDTH, 20,
                Text.literal(sliderText(label, get.getAsDouble(), step, unit)),
                (get.getAsDouble() - min) / (max - min)) {
            @Override
            protected void updateMessage() {
                setMessage(Text.literal(sliderText(label, snapped(), step, unit)));
            }
    ...
    private TextFieldWidget field(String initial, int maxLength, Consumer<String> onChange) {
        TextFieldWidget box = new TextFieldWidget(textRenderer, 0, 0, SettingsList.CONTROL_WIDTH, 18, Text.empty());
        box.setMaxLength(maxLength);
        box.setText(initial);
        box.setCursor(0, false);
        box.setSelectionEnd(0);
        box.setChangedListener(onChange);
        return box;
    }

    private static ClickableWidget tip(ClickableWidget w, String text) {
    ...
    protected void init() {
        ...
        TabNavigationWidget.Builder bar = TabNavigationWidget.builder(tabManager, width);
        bar.tabs(
        ...
        tabBar = addDrawableChild(bar.build());
        tabBar.init();

        DirectionalLayoutWidget footer = DirectionalLayoutWidget.horizontal().spacing(8);
        footer.add(ButtonWidget.builder(Text.literal("Reset to defaults"), b -> resetDefaults())
                .width(150).build());
        footer.add(ButtonWidget.builder(Text.literal("Done"), b -> close()).width(150).build());
        footer.forEachChild(this::addDrawableChild);
        footer.refreshPositions();
        net.minecraft.client.gui.widget.SimplePositioningWidget.setPos(
    ...
    private Tab page(String title, Consumer<SettingsList> build) {
        SettingsList list = new SettingsList(client, width, height, 0, 0);
        build.accept(list);
        lists.add(list);
        return new ListTab(Text.literal(title), list);
    }
    ...
    private final class ListTab implements Tab {
        ...
        ListTab(Text title, SettingsList list) {
            this.title = title;
            this.list = list;
        }
        ...
        @Override
        public void forEachChild(Consumer<ClickableWidget> visitor) {
            visitor.accept(list);
        }

        @Override
        public void refreshGrid(ScreenRect area) {
            list.position(area.getRight() - area.getLeft(), area.getBottom() - area.getTop(), area.getTop());
        }
    }
    ...
    protected void refreshWidgetPositions() {
        ...
        tabBar.init();
        int top = tabBar.getNavigationFocus().getBottom();
        tabManager.setTabArea(new ScreenRect(0, top, width, height - top - FOOTER_H));
    }
    ...
    private void translateTab(TEConfig cfg, SettingsList l) {
        ...
        l.wide(ButtonWidget.builder(Text.literal("Test connection"), b -> runTest(status))
    ...
    private void aiTab(TEConfig cfg, SettingsList l) {
        ...
        SettingsList.StatusRow[] status = {l.status()};
        l.pair(ButtonWidget.builder(Text.literal("Test connection"), b -> runAiTest(status[0]))
                        .dimensions(0, 0, 150, 20).build(),
        ...
        l.wide(ButtonWidget.builder(Text.literal(cfg.aiSystemPrompt.isBlank()
                                ? "Edit instructions" : "Edit instructions (custom)"),
                        b -> client.setScreen(new PromptScreen(this)))
                .dimensions(0, 0, 150, 20).build());
    }
    ...
    private void advancedTab(TEConfig cfg, SettingsList l) {
        ...
        l.wide(ButtonWidget.builder(Text.literal("Manage cache…"),
                b -> client.setScreen(new CacheScreen(this))).dimensions(0, 0, 150, 20).build());
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        return (tabBar != null && tabBar.trySwitchTabsWithKey(keyCode))
